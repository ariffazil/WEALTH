...
const handlers = {
  ...
  '/api/oil/snapshot': async () => {
    ...
    const d = validateSnapshot(await runPython('snapshot'), 'oil');
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const isSnapshot = parsed.pathname === '/api/oil/snapshot' || parsed.pathname === '/api/snapshot';
    if (isSnapshot) {
      res.writeHead(503, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({
        schema: 'wealth.snapshot.v1', asset: 'oil', error: true,
