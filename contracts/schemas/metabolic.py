"""
WEALTH Metabolic Output Schemas — Phase 2 Adoption of metabolic.v1
══════════════════════════════════════════════════════════════════════════════════

Canonical source: arifOS commit 3c64960e
Source path: arifosmcp/schemas/metabolic.py
Contract hash: a5826a9eb1182c4f212fda1baa55ff9f
Schema version: metabolic.v1

This is a BYZANTINE-FAULT-TOLERANT canonical copy.
arifOS is the SOLE source of truth for the metabolic contract.
Do not modify this file — updates must come from arifOS only.
...
class CandidateMeaning(BaseModel):
    ...
    primary_interpretation: str = Field(
        default="",
        description="Most likely meaning given current evidence",
    )
    ...
    meaning_confidence_band: tuple[float, float] = Field(
        default=(0.0, 1.0),
        description="F07 HUMUMITY: uncertainty range for this meaning claim",
    )
...
class ConstraintCheck(BaseModel):
    ...
    check_passed: bool = Field(
        default=False,
        description="Did the candidate meaning pass this constraint?",
    )
...
class ModelUpdate(BaseModel):
    ...
    state_before: dict[str, Any] = Field(
        ...
        description="Model state before this witness was applied",
    )
    incoming_witnesses: list[str] = Field(
        default_factory=list,
        description="Which witness IDs contributed to this update",
    )
    ...
    state_after: dict[str, Any] = Field(
        default_factory=dict,
        description="Model state after this witness was applied",
    )
    confidence_delta: float = Field(
        default=0.0,
        description="Change in model confidence from this update",
    )
    audit_receipt: str = Field(
        default="",
        description="VAULT999 reference for this model update",
    )
...
class AbstractionGuard(BaseModel):
    ...
    misuse_risk: str = Field(
        default="",
        description="Known risks if this metaphor is over-extended",
    )
...
class UncertaintyBand(BaseModel):
    ...
    omega_0: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Base uncertainty (Ω₀)",
    )
    ...
    key_missing_evidence: list[str] = Field(
        default_factory=list,
        description="What evidence would most reduce uncertainty",
    )
...
class EvidenceFreshness(BaseModel):
    ...
    staleness_risk: StalenessRisk = Field(
        default=StalenessRisk.MODERATE,
        description="Likely rate of evidence decay",
    )
    requires_refresh: bool = Field(
        default=False,
        description="Should this evidence be re-fetched before use?",
    )
    refresh_recommendation: str = Field(
        default="",
        description="Human-readable recommendation for refresh",
    )
...
class CrossOrganHandoff(BaseModel):
    ...
    handoff_payload: dict[str, Any] = Field(
        default_factory=dict,
        description="Structured data to pass to the next organ",
    )
    ...
    blocked_reason: str = Field(
        default="",
        description="Why each blocked organ was excluded",
    )
    confidence_at_handoff: ConfidenceLevel = Field(
        default=ConfidenceLevel.MODERATE,
        description="Confidence level at handoff",
    )
...
class OrganConflict(BaseModel):
    ...
    conflict_domain: str = Field(
        default="",
        description="earth | wealth | governance | health | law | system",
    )
    ...
    recommended_action: str = Field(
        default="",
        description="What should happen given this conflict",
    )
...
class MetabolicOutput(BaseModel):
    ...
    witness_type: WitnessType | None = Field(
        default=None,
        description="Primary witness category",
    )
    ...
    decoded_entities: list[DecodedEntity] = Field(
        default_factory=list,
        description="Objects extracted from witnesses",
    )

    # ── Contrast layer ────────────────────────────────────────────────────────
    anomalous_contrasts: list[AnomalousContrast] = Field(
        default_factory=list,
        description="Deviations from expected background",
    )
    ...
    constraints_checked: list[ConstraintCheck] = Field(
        default_factory=list,
        description="Rules, physics, law, ethics verified",
    )

    # ── Model update layer ────────────────────────────────────────────────────
    model_updates: list[ModelUpdate] = Field(
        default_factory=list,
        description="Proposed state changes to domain models",
    )
    model_target: ModelTarget | None = Field(
        default=None,
        description="Which domain model was updated",
    )

    # ── Abstraction guard ─────────────────────────────────────────────────────
    abstraction_guard: AbstractionGuard | None = Field(
        default=None,
        description="If metaphors were used, guard against hallucination",
    )
    ...
    required_next_tests: list[str] = Field(
        default_factory=list,
        description="What tests would most improve confidence",
    )
    ...
    audit_receipt: str = Field(
        default="",
        description="VAULT999 reference for this metabolic cycle",
    )
    ...
    recommendation_only: bool = Field(
        default=True,
        description="AI proposes only — has not been ratified by human",
    )
    ...
    human_final_authority: str = Field(
        default="Arif",
        description="Who has final say on this output?",
    )
    requires_888_judge: bool = Field(
        default=False,
        description="Does this output require 888_JUDGE before action?",
    )
    ...
    class Config:
        json_schema_extra = {
            "description": (
                ...
                "DITEMPA BUKAN DIBERI — metabolic.v1"
            ),
            "schema_version": "metabolic.v1",
