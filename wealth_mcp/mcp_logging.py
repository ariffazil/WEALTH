"""
wealth_mcp.mcp_logging — MCP logging utility (FREEZE — SEP-2577)
...
Canonical ops path: stderr + structured receipts + HOLD_CANDIDATE (arifOS).
...
_stderr = logging.getLogger("wealth.mcp_log")
if not _stderr.handlers:
    _h = logging.StreamHandler(sys.stderr)
    _h.setFormatter(logging.Formatter("%(asctime)s %(levelname)s [wealth.mcp] %(message)s"))
...
async def emit_mcp_log(
    ...
    *,
    ...
    logger_name: str = "wealth.envelope",
