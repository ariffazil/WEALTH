...
PUBLIC_SURFACE_WHITELIST = {
    ...
    # L4 — SAF-organ merged stat tools (2026-06-02)
    "wealth_stat_regress",
    "wealth_stat_anova",
    "wealth_stat_correlate",
    "wealth_stat_power",
    "wealth_stat_effect_size",
    "wealth_stat_chi_square",
    "wealth_stat_compare_groups",
}
...
# ════════════════════════════════════════════════════════════════════════
# SAF-organ merged statistical tools (2026-06-02)
# ════════════════════════════════════════════════════════════════════════
# Re-expose 7 statistical tools from arifOS/core/shared/saf_stats/.
...
#    (WELL GEOX WEALTH) finish the refactor and compression and lower
#    the entropy"
# ════════════════════════════════════════════════════════════════════════

import os as _os_saf
import sys as _sys_saf
import uuid as _uuid_saf
from pathlib import Path as _Path_saf
import pandas as _pd_saf

# Add arifOS kernel to sys.path (shared lib lives in core/shared/saf_stats/)
_arifos_root = _os_saf.environ.get("ARIFOS_ROOT", "/root/arifOS")
if _arifos_root not in _sys_saf.path:
    _sys_saf.path.insert(0, _arifos_root)

# F6 PRIVACY: WEALTH-owned sandbox for stat temp CSVs
_WEALTH_SAF_ROOT = _Path_saf(
    _os_saf.environ.get("WEALTH_SAF_DATA_ROOT", "/tmp/wealth_saf")
)
_WEALTH_SAF_ROOT.mkdir(parents=True, exist_ok=True)
_os_saf.environ.setdefault("SAF_DATA_ROOT", str(_WEALTH_SAF_ROOT))
...
def _wealth_saf_write(df):
    fpath = _WEALTH_SAF_ROOT / f"wealth_{_uuid_saf.uuid4().hex[:12]}.csv"
    df.to_csv(fpath, index=False)
    return fpath


def _wealth_saf_cleanup(fpath):
    try:
        fpath.unlink()
    except OSError:
        pass
...
@mcp.tool(name="wealth_stat_regress")
def wealth_stat_regress(
    y: list,
    x_columns: dict,
    family: str = "ols",
    robust: bool = False,
    ...
    df = _pd_saf.DataFrame({"y": y, **x_columns})
...
@mcp.tool(name="wealth_stat_anova")
def wealth_stat_anova(groups: list, alpha: float = 0.05) -> Any:
...
@mcp.tool(name="wealth_stat_correlate")
def wealth_stat_correlate(x: list, y: list, method: str = "pearson") -> Any:
...
@mcp.tool(name="wealth_stat_power")
def wealth_stat_power(
    test: str = "t",
...
@mcp.tool(name="wealth_stat_effect_size")
def wealth_stat_effect_size(x: list, y: list, kind: str = "cohens_d") -> Any:
...
@mcp.tool(name="wealth_stat_chi_square")
def wealth_stat_chi_square(observed: list, expected: Optional[list] = None) -> Any:
...
@mcp.tool(name="wealth_stat_compare_groups")
def wealth_stat_compare_groups(
...
WEALTH_PUBLIC_TOOL_ORDER = (
    # L0 — Kernel Surface
    "mcp_health_check",
    # SAF-organ merged stat tools (2026-06-02) — see saf_stat_integration.py
    "wealth_stat_regress",
    "wealth_stat_anova",
    "wealth_stat_correlate",
    "wealth_stat_power",
