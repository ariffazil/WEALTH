...
def _is_observation_stale(
    observation_time: Optional[str], frequency: Optional[str]
) -> bool:
...
class IngestRegistry:
    ...
    def fetch(
        ...
        **kwargs,
        ...
        try:
            ...
            self._health.record_attempt(
                source, False, latency_ms, error_message=str(exc)
            )
            return {
                "records": [],
    ...
    def snapshot(
        self, entity_code: str, sources: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        for source in sources:
            ...
            for spec in source_specs:
                ...
                # Keep only latest NON-NULL observation per series for snapshot
                # (WB returns 2025=null for unreleased annual data, so use latest valid value)
                non_null = [r for r in result["records"] if r.get("value") is not None]
                latest = None
                if non_null:
                    latest = max(non_null, key=lambda r: r.get("observation_time", ""))
                snapshot[key] = latest
    ...
    def reconcile(self, entity_code: str) -> Dict[str, Any]:
        ...
        snapshot_result = self.snapshot(
            entity_code, sources=["WorldBank", "Ember", "OWID"]
        )
